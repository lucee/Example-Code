...
- classic: classic view with html/css/javascript"},
	...
	string function html( required struct meta,
		...
		var indent = repeatString(variables.TAB, arguments.level);
			...
			if(arguments.level EQ 0){
				...
				head&=('#prefix# table {font-family:Verdana, Geneva, Arial, Helvetica, sans-serif; font-size:11px; empty-cells:show; color:#invert(colors.fontColor)#; border-collapse:collapse;}' & variables.NEWLINE);
				head&=('#prefix# td {border:1px solid #invert(colors.borderColor)#; vertical-align:top; padding:2px; empty-cells:show;}' & variables.NEWLINE);
				...
				loop struct="#arguments.meta.colors#" index="local.k" item="local.v" {
					...
					var fc=(bc);
					head&="#prefix# td.n#variables.colorKeys[k]# {color:#invert(fc)#;border-color:#invert(bc)#;background-color:#invert(v.normalColor)#;}"& variables.NEWLINE;
					head&="#prefix# td.h#variables.colorKeys[k]# {color:#invert(fc)#;border-color:#invert(bc)#;background-color:#invert(v.highLightColor)#;}"& variables.NEWLINE;
				}
	...
	string function classic( required struct meta,
		...
		var normalColor = "white";
			...
			if(arguments.level EQ 0){
				...
				rtn&=( '#prefix# table {font-family:Verdana, Geneva, Arial, Helvetica, sans-serif; font-size:11px; empty-cells:show; color:#invert(colors.fontColor)#; border: 2px solid black; border-collapse:collapse;}' & variables.NEWLINE);
				...
				loop struct="#arguments.meta.colors#" index="local.k" item="local.v" {
					...
					variables.colorKeys[k]=count++;
					rtn&="#prefix# td.n#variables.colorKeys[k]# {background-color:black;border-color:#invert(borderColor)#; color:white;cursor:pointer;}"& variables.NEWLINE;
					rtn&="#prefix# td.h1#variables.colorKeys[k]# {background-color:#invert(h1Color)#;border-color:#invert(borderColor)#; color:black;cursor:pointer;}"& variables.NEWLINE;
					rtn&="#prefix# td.h2#variables.colorKeys[k]# {background-color:#invert(h2Color)#;border-color:#invert(borderColor)#; color:white;cursor:pointer;}"& variables.NEWLINE;
				}
	...
	string function simple( required struct meta,
		...
		if(arguments.level EQ 0){
			...
			if(structKeyExists(arguments.meta, 'title')){
				...
				rtn&=('<tr>');
				rtn&=('<td colspan="#columnCount#" bgcolor="#invert(variables.colors.highLightColor)#">');
				...
				loop query="arguments.meta.data" {
					...
					for(col=1; col LTE columnCount-1; col++) {
						...
						if(isStruct(node)) {
							...
							rtn&=('<td bgcolor="#invert(doHighlight(arguments.meta,c)?variables.colors.highLightColor:variables.colors.normalColor)#">' );
							rtn&=(value);
							rtn&=('</td>');
						}
						else {
							rtn&=('<td bgcolor="#invert(doHighlight(arguments.meta,c)?variables.colors.highLightColor:variables.colors.normalColor)#">' & HTMLEditFormat(node) & '</td>');
						}
						c *= 2;
					}
					rtn&=('</tr>');
	...
	private function invert(required string color) {
		local.org=color;
	    try{
	    	if(left(color,1)=='##') color=mid(color,2);
		    local.str='';
		    loop from=1 to=stringLen(color) item="local.i" {
		        str&=FormatBaseN(15-parseNumber(color[i],'hex'),16);
		    }
		    return "##"&str;
	    }
	    catch(local.e) {}
